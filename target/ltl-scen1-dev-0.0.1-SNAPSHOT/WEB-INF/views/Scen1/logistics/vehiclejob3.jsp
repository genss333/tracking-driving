...
//======================================================== Add Met API ===================================================
	...
	const getOldLine = async()=>{
		...
		setInterval( async() => {
			...
			data.result.dashedLine.map((val,i)=>{
				...
				//console.log(val);
				val.map((val,i)=>{
					dashedLines.push(L.latLng(val[1],val[0]))
				})
				...
				arrdashedline.splice(i, 1,drawdashline(dashedLines,colors[i]))
				homes.splice(i, 1,L.marker(dashedLines[dashedLines.length-1], { icon: IconHouse, title: 'Hover Text' }).addTo(waypointmap).bindPopup("data"))
	...
	const drawline = (latlng,color)=>{
    	return polyline1 = L.polyline(latlng, {color: color, weight: '3',opacity: 0.6}).addTo(waypointmap);
	}
