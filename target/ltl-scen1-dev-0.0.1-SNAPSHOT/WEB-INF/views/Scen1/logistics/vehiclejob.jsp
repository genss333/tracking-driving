		...
		<div class="d-flex flex-column" id="content-wrapper">
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div class="tab-content shadow bg-white rounded " id="myTabContent" style="height: 70vh;  overflow: hidden; margin-bottom: 2%;">
						<div class="tab-pane shadow bg-white rounded  fade show active"style="height: 70vh; margin-bottom: 2%"" id="mainmap" role="tabpanel"></div>
						<div class="tab-pane shadow  bg-white rounded  fade active"style="height: 70vh; margin-bottom: 2%"" id="waypointmap"  role="tabpanel"></div>
					</div>
...
/* console.log(RGB(255,0,0)); */
        ...
        const {data} = await axios.get('${pageContext.request.contextPath}/provider/findroute/${jobsectionId}')
         $(document).ready( function () {
            ...
            $("#jobfault").html(data[0].countjob);
            $("#vehicle").html("ข้อมูลการขนส่ง(BestExpress 1)");
        })
        let latlngs=[]
        for (let i = 0; i < data[0].order.length-1; i++) {	
            ...
            L.marker(L.latLng(data[0].order[i].lat, data[0].order[i].lng), { icon: IconHouse, title: 'Hover Text' }).addTo(mainmap).bindPopup(data[0].order[i].index+": "+data[0].order[i].firstname+" "+data[0].order[i].lastname)
						.openPopup();
            L.marker(L.latLng(data[0].order[i].lat, data[0].order[i].lng), { icon: IconHouse, title: 'Hover Text' }).addTo(waypointmap).bindPopup(data[0].order[i].index+": "+data[0].order[i].firstname+" "+data[0].order[i].lastname)
			.openPopup();
        ...
        L.marker(L.latLng(data[0].order[data[0].order.length-1].lat, data[0].order[data[0].order.length-1].lng), { icon: IconWarehouse, title: 'Hover Text' }).addTo(mainmap).bindPopup(data[0].order[data[0].order.length-1].firstname)
						.openPopup();
        L.marker(L.latLng(data[0].order[data[0].order.length-1].lat, data[0].order[data[0].order.length-1].lng), { icon: IconWarehouse, title: 'Hover Text' }).addTo(waypointmap).bindPopup(data[0].order[data[0].order.length-1].firstname)
		.openPopup();
    }
    const getwaypoint = async()=>{
    	console.log("getwaypoint Done")
...
//======================================================== Add Met API ===================================================
	const work = async()=>{
		...
		const {data} = await axios.get('${pageContext.request.contextPath}/tracking/tracking/333')
		L.marker(L.latLng(data.result.polyline[data.result.polyline.length-1][1],data.result.polyline[data.result.polyline.length-1][0]), { icon: IconWarehouse }).addTo(waypointmap);
		L.marker(L.latLng(data.result.dashline[data.result.dashline.length-1][1],data.result.dashline[data.result.dashline.length-1][0]), { icon: IconHouse }).addTo(waypointmap);
		let drawlines;
		let drawdashlines;
		let markers;
		setInterval(async() => {
			console.log("Run");
			if(drawlines && drawdashlines && markers){
			drawdashlines.remove()
			drawlines.remove()
			markers.remove()
		}
		const {data} = await axios.get('${pageContext.request.contextPath}/tracking/tracking/333')
		...
		data.result.dashline.map((val,i)=>{
			dashline.push(L.latLng(val[1],val[0]))
		})
		data.result.polyline.map((val,i)=>{
			polyline.push(L.latLng(val[1],val[0]))
		})
		drawlines = drawline(polyline,"#669DF6")
		drawdashlines = drawdashline(dashline,"#78909C")
		markers = L.marker(polyline[0], { icon: Iconvehicle }).addTo(waypointmap);
		}, 5000);
		
	}

	const getOldLine = async()=>{
		console.log("getOldLine Done");
		let polyLines = []
		let dashedLines = []
		...
		setInterval( async() => {
			...
			const {data} = await axios.get('${pageContext.request.contextPath}/tracking/getoldline/${jobsectionId}')
			console.log(data);

			polyLines = []
			data.result.polyLine.map((val,i)=>{
				polyLines.push(L.latLng(val[1],val[0]))
			})

			data.result.dashedLine.map((val,i)=>{
				dashedLines=[]
				//console.log(val);
				let newlatlng = val.map((val,i)=>{
					return[val[1],val[0]]
				})
				console.log(i+","+arrdashedline.length);
				if(arrdashedline[i]){
					arrdashedline[i].remove()
					homes[i].remove()
				}
				arrdashedline.splice(i, 1,drawdashline(newlatlng,colors[i]))
				homes.splice(i, 1,L.marker(newlatlng[newlatlng.length-1], { icon: IconHouse, title: 'Hover Text' }).addTo(waypointmap).bindPopup("data"))
			...
			polyline = drawline(polyLines,"#FF0000")
			vehicle = L.marker(polyLines[0], { icon: Iconvehicle, title: 'Hover Text' }).addTo(waypointmap).bindPopup("Car")
		}, 2000);
...
const getplan= async()=>{
	let timereset = 10000;
	let interval;
	const {data} = await axios.get('${pageContext.request.contextPath}/tracking/getplan/${jobsectionId}')
	if(!plandashedline){
		data.result.dashedLine.map((val,i)=>{
			plandashedline = val.map(val=>{
				return[val[1],val[0]]
			})
			drawdashline(plandashedline,colors[i])
    	})
	}
    
	interval = setInterval(()=>{
		if(data.result.polyLine){
			let polyline = data.result.polyLine.map(val=>{
			return [val[1],val[0]]
		})
		drawline(polyline,"#FF0000")
		markers = L.marker(polyline[polyline.length-1], { icon: Iconvehicle }).addTo(waypointmap);
		starttime = data.result.endTime
		console.log("getplan Done");
		clearInterval(interval)
		getnextline()
		} else{
			console.log("Waiting for Polyline");
			clearInterval(interval)
			getplan()
...
const getnextline = async()=>{
	...
	setInterval(async() => {
		...
		console.log(payload);
		const {data} = await axios.post('${pageContext.request.contextPath}/tracking/getnextline',payload)
		if(data.status == "ok"){
			let polyline = data.result.polyLine.map(val=>{
				return [val[1],val[0]]
			})
...
//================ 0.0.4 ================
	...
	const drawline = (latlng,color)=>{
    	return polyline1 = L.polyline(latlng, {color: color, weight: '6',opacity: 0.6}).addTo(waypointmap);
	}
	const drawdashline = (latlng,color)=>{
    	return polyline1 = L.polyline(latlng, {color: color, weight: '3', dashArray: '10, 13', dashOffset: '0'}).addTo(waypointmap);
	}
	//work()
	// getOldLine()
	getplan()
