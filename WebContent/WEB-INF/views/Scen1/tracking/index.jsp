	...
	<script>
		...
		L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
         ...
         $(document).ready( function () {
            vehicles = $('#datatable-vehicles').DataTable();
            for (let i = 0; i < 5; i++) {
                vehicles.row.add([
                        i+1,
                        Math.floor(Math.random() * 3000),
                        "รถส่งพัสดุ "+Math.floor(Math.random() * 10),
                        "คนขับ "+Math.floor(Math.random() * 10),
						`<form action="${pageContext.request.contextPath}/tracking/trackingdetail" method="post">
                            <button type="submit" class="btn btn-success" name="id" value="`+(i+1)+`"><i class="fas fa-map-marked-alt"></i> จุดหมาย</button>
                            </form>`
                    ]).draw( false );
            }
            } ); 
        ...
        const runvehicle = ()=>{
            ...
            setInterval(() => {
                ...
                if(val[j]){
                    ...
                    vehicle = L.marker(val[j], { icon: Iconvehicle }).addTo(map)
